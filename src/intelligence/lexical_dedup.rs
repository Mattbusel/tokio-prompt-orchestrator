...
//! # LexicalDedup — lexical similarity deduplicator
//!
...
/// Backward-compatible type alias for [`LexicalDedup`].
///
/// The old name `SemanticDedup` was misleading because the implementation
/// uses hash-projection cosine + Jaccard word overlap, not semantic embeddings.
#[deprecated(note = "Renamed to LexicalDedup")]
pub type SemanticDedup = LexicalDedup;

impl LexicalDedup {
    pub fn new(config: SemanticDedupConfig) -> Self {
        warn!(
            "LexicalDedup uses hash-projection similarity, not semantic embeddings"
        );
        Self {
...
mod tests {
    ...
    fn test_embed_deterministic_same_text_same_result() {
        let a = LexicalDedup::embed("hello world", 64);
        let b = LexicalDedup::embed("hello world", 64);
    ...
    fn test_embed_dimension_matches_requested() {
        for dim in [1, 16, 64, 128, 256] {
            let emb = LexicalDedup::embed("test text", dim);
            assert_eq!(
                emb.len(),
                dim,
    ...
    fn test_embed_different_texts_produce_different_embeddings() {
        let a = LexicalDedup::embed("the quick brown fox", 64);
        let b = LexicalDedup::embed("jumped over the lazy dog", 64);
        assert_ne!(a, b, "different texts should produce different embeddings");
    }

    #[test]
    fn test_embed_normalized_to_unit_vector() {
        let emb = LexicalDedup::embed("normalize me please", 64);
    ...
    fn test_embed_single_word_normalized() {
        let emb = LexicalDedup::embed("singleton", 128);
        let magnitude: f64 = emb.iter().map(|v| v * v).sum::<f64>().sqrt();
        assert!(
            (magnitude - 1.0).abs() < 1e-6,
    ...
    fn test_cosine_similarity_identical_vectors_returns_one() {
        ...
        let sim = LexicalDedup::cosine_similarity(&v, &v);
        assert!(
            (sim - 1.0).abs() < 1e-9,
    ...
    fn test_cosine_similarity_orthogonal_vectors_near_zero() {
        ...
        let a = vec![1.0, 0.0];
        let b = vec![0.0, 1.0];
        let sim = LexicalDedup::cosine_similarity(&a, &b);
        assert!(
            sim.abs() < 1e-9,
    ...
    fn test_lookup_empty_index_returns_none() {
        let dedup = LexicalDedup::new(SemanticDedupConfig::default());
    ...
    fn test_index_full_without_fallback_returns_error() {
        ...
        let dedup = LexicalDedup::new(cfg);
    ...
    fn test_index_full_with_fallback_evicts_lru_entry() {
        ...
        let key_a = LexicalDedup::make_key("prompt-a");
        assert!(
    ...
    fn test_lookup_miss_increments_miss_counter() {
        ...
        let dedup2 = LexicalDedup::new(cfg);
    ...
    fn test_embed_dim_one() {
        let emb = LexicalDedup::embed("test", 1);
        assert_eq!(emb.len(), 1);
    ...
    fn test_eviction_preserves_high_hit_count_entries() {
        ...
        assert_eq!(guard.len(), 3);
        let keys: Vec<&str> = guard.iter().map(|e| e.key.as_str()).collect();
        let beta_key = LexicalDedup::make_key("beta");
        assert!(
